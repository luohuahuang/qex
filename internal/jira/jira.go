...
func AddComment(client *jira.Client, issueId string, buildExporter protocol.JenkinsBuildExporter) error {
	...
	if err != nil {
		monitor.SendAlert(errors.New(fmt.Sprintf("%s: %s", issueId, err.Error())))
		return err
	} else {
		monitor.SendAlert(errors.New(fmt.Sprintf("INFO: QEX just commented JIRA ticket %s with job info. Go to JIRA to verify...", issueId)))
	}
	log.Println(resp)
	return nil
}
